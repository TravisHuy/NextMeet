...
class NoteRepository @Inject constructor(private val noteDao: NoteDao) {
    ...
    suspend fun deleteNote(noteId:Int) : Result<Unit>{
        return try{
            val note = noteDao.getNoteById(noteId) ?: return Result.failure(IllegalArgumentException("ghi chú không tồn tại"))
            Result.success(Unit)
        }
        catch(e:Exception){
            Result.failure(e)
    ...
    suspend fun togglePin(noteId:Int):Result<Boolean>{
        return try{
            val note = noteDao.getNoteById(noteId) ?: return Result.failure(IllegalArgumentException("Ghi chú không tồn taại"))
